...
The Docker and Pylint workflows remain useful CI signals, but they are not part of the minimum checks defined by issue #140 unless branch-protection policy is deliberately expanded.
...
3. A dependency change rejected by Dependency Review makes that check fail.
4. SonarCloud analysis runs for repository pull requests, and a failed SonarQube Cloud Quality Gate is visible as a failing check.
5. GitHub prevents merging while any configured required check is failing or pending.
